...
import com.infoaa.sharing.etracker.service.ExpenseService;
import com.infoaa.sharing.etracker.service.ImageService;
import com.infoaa.sharing.etracker.util.PieChart;
...
public class TrackerController {
	...
	public void showImage(@RequestParam("id") Integer expenseId, HttpServletResponse response,HttpServletRequest request)
		...
		Optional<UploadImage> uploadImage = imageService.getUploadImage(expenseId);
		response.setContentType("image/jpeg, image/jpg, image/png, image/gif");
		if (uploadImage.isPresent()) {
			response.getOutputStream().write(uploadImage.get().getImage());
		}
	...
	public String EditController(@PathVariable("expenseId") String expId, Model model) {
		...
		if (expense.isPresent()) {
			Optional<Category> category = categoryService.getCategoryById(expense.get().getCategoryId());
			if (category.isPresent()) {
				model.addAttribute("StringCategory", category.get().getCategory());
			}
		}
		Optional<UploadImage> image = imageService.getUploadImage(expenseId);
		List<String> categories = categoryService.getOnlyCategories();
		model.addAttribute("expense", expense);
		model.addAttribute("categories", categories);
		if (image.isPresent()) {
			model.addAttribute("onlyImage",image.get().getImage());
		}
		return "edit";
	}
