package com.infoaa.sharing.api.utils;
...
import com.infoaa.sharing.api.model.Member;
import com.infoaa.sharing.api.model.Member.Members;
import com.infoaa.sharing.api.model.Payment;

public class DebtCalculator {

	public  static  int  MAX_MEMBERS_FOR_OPTIMAL = 1000; 
   ...
   public static synchronized List<Debt> calculate(List<Member> members, BigDecimal tolerance, String currency) {
       ...
       // for 18 and more members algorithm would take long time, so just use basic algorithm
   ...
   public static synchronized BigDecimal updateBalances(List<Member> members, List<Payment> payments,
       ...
       for (Payment payment : payments) {
           ...
           boolean settlement = (payment.transfer || payment.purpose.equals("Settlement"));
           ...
           for (int i = 0; i < payment.whoPaid.size(); i++) {
               ...
               if (idsToMembers.get(whoPaid) == null) {
                   Member newMember = new Member(whoPaid, "MissingMember" + " " + missingMembers,
                           "", groupId, null);
           ...
           for (long id : payment.forWho) {
               ...
               if (idsToMembers.get(id) == null) {
                   Member newMember = new Member(id, "MissingMember" + " " + missingMembers, "",
                           groupId, null);
                   members.add(newMember);
